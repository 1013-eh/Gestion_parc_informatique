<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Créer une marque</h2>
    </x-slot>
    <div class="max-w-2xl mx-auto py-6">
        <form method="POST" action="{{ route('admin.marques.store') }}" class="bg-white shadow rounded-lg p-6">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                <label for="" class="block text-gray-700 font-semibold mb-2">Nom de la marque</label>
                <input type="text" name="nom_marque" placeholder="Nom de la marque" class="w-full border border-gray-300 rounded px-3 py-2">
                @error('nom_marque')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
