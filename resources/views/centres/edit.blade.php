...
@section('title', 'Modifier un centre')
@section('page-title', '✏️ Modifier un centre')
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <h2 class="text-xl font-semibold text-gray-800 mb-6">
                    <i class="fas fa-edit text-yellow-600 mr-2"></i> Modifier le centre : {{ $centre->code_bureau }}
                </h2>
                ...
                <form action="{{ route('centres.update', $centre->code_bureau) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            ...
                            <input type="number" 
                                   ...
                                   value="{{ old('code_bureau', $centre->code_bureau) }}"
                                   required>
                            @error('code_bureau')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('nom_centre', $centre->nom_centre) }}"
                                   required>
                            @error('nom_centre')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select name="id_region" 
                                ...
                                @foreach($regions as $region)
                                    <option value="{{ $region->id_region }}" {{ old('id_region', $centre->id_region) == $region->id_region ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('adresse_ip', $centre->adresse_ip) }}"
                                   required>
                            @error('adresse_ip')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type_consultation" 
                                ...
                                <option value="GLOBAL" {{ old('type_consultation', $centre->type_consultation) == 'GLOBAL' ? 'selected' : '' }}>🌍 Consultation globale</option>
                                <option value="PAR_CENTRE" {{ old('type_consultation', $centre->type_consultation) == 'PAR_CENTRE' ? 'selected' : '' }}>🏢 Consultation par centre</option>
                                <option value="ADMIN" {{ old('type_consultation', $centre->type_consultation) == 'ADMIN' ? 'selected' : '' }}>🔒 Administrateur</option>
                            </select>
